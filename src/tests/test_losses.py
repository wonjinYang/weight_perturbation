...
class TestLosses(unittest.TestCase):
    ...
    def test_multi_marginal_ot_loss_basic(self):
        ...
        loss = multi_marginal_ot_loss(gen_out, evidence_list, weights=None, blur=0.06, entropy_lambda=0.012)
        self.assertIsInstance(loss, torch.Tensor)
        self.assertEqual(loss.dim(), 0)
        self.assertTrue(torch.isfinite(loss))
    ...
    def test_multi_marginal_ot_loss_weights(self):
        ...
        weights = [0.7, 0.3]
        loss = multi_marginal_ot_loss(gen_out, evidence_list, weights=weights)
        self.assertTrue(torch.isfinite(loss))
    ...
    def test_multi_marginal_ot_loss_errors(self):
        ...
        """
        with self.assertRaises(ValueError):
            multi_marginal_ot_loss(torch.randn(10, 2), [])  # Empty evidence

        with self.assertRaises(ValueError):
            multi_marginal_ot_loss(torch.randn(10, 2), [torch.randn(10, 2)], weights=[0.5, 0.5])  # Weights mismatch

        with self.assertRaises(ValueError):
            multi_marginal_ot_loss(torch.randn(10, 2), [torch.randn(10, 3)])  # Dim mismatch
